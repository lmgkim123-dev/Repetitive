...
def build_task_display_df(task_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df["TA 조치 요약"] = display_df.get("TA 조치사항", "").fillna("").astype(str).apply(lambda x: _truncate_multiline(x, 220))
    display_df["권고 요약"] = display_df.get("추후 권고사항", "").fillna("").astype(str).apply(lambda x: _truncate_multiline(x, 180))
    preferred_cols = [
...
# main UI
# -------------------------------------------------
analysis_mode = st.sidebar.radio("분석 대상", options=["고정장치", "배관"], index=0)

if analysis_mode == "고정장치":
    st.subheader("고정장치 분석")
    uploaded_files = st.file_uploader(
        "PDF / Excel / CSV / TXT 파일 업로드",
        type=["pdf", "xlsx", "xls", "xlsm", "xlsb", "csv", "txt"],
        accept_multiple_files=True,
        key="fixed_uploader",
    )

    if st.button("고정장치 분석 실행", type="primary", disabled=not uploaded_files):
        st.session_state.pop("fixed_analysis_result", None)
        run_fixed_analysis(uploaded_files)

    result = st.session_state.get("fixed_analysis_result")
    if result:
        overview_df = result["overview_df"]
        repeat_cases = result["repeat_cases"]
        ...
        c1.metric("업로드 파일 수", f"{result['uploaded_count']:,}")
        ...
        occurrence_option = st.radio(
            ...
            index=0,
            horizontal=True,
            key="fixed_occurrence_option",
        )
        selected_categories = st.multiselect(
            "카테고리 재선택",
            options=CATEGORY_ORDER,
            default=CATEGORY_ORDER,
            key="fixed_selected_categories",
        )
        ...
        with d2:
            st.download_button(
                "전체 결과 Excel 다운로드",
                data=result["full_excel_bytes"],
                file_name=result["full_excel_name"],
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )

        tabs = st.tabs(["과제후보_등록형식", "카테고리별_발췌", "반복정비_Case", "연도별_정비이벤트"])
        with tabs[0]:
            st.dataframe(_sanitize_df_for_display(display_task_df), use_container_width=True, height=420)
        with tabs[1]:
            st.dataframe(_sanitize_df_for_display(category_extract_df), use_container_width=True, height=360)
        with tabs[2]:
            st.dataframe(_sanitize_df_for_display(cases_df), use_container_width=True, height=260)
        with tabs[3]:
            st.dataframe(_sanitize_df_for_display(events_df), use_container_width=True, height=360)
    ...
    with col2:
        trouble_file = st.file_uploader(
            ...
            type=["xlsx", "xls", "xlsm", "xlsb", "csv"],
            accept_multiple_files=False,
            key="piping_trouble_uploader",
        )

    if st.button("배관 분석 실행", type="primary", disabled=not (history_file and trouble_file)):
        st.session_state.pop("piping_analysis_result", None)
        run_piping_analysis(history_file, trouble_file)

    result = st.session_state.get("piping_analysis_result")
    if result:
        summary_df = result["summary_df"]
        repeat_df = result["repeat_df"]
        occurrences_df = result["occurrences_df"]
        excluded_df = result["excluded_df"]

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("정비이력 파일", result["history_name"])
        c2.metric("Trouble 파일", result["trouble_name"])
        c3.metric("교체 occurrence", f"{len(occurrences_df):,}")
        c4.metric("반복 Line Number", f"{len(repeat_df):,}")

        st.subheader("배관 필터 결과")
        count_mode = st.radio(
            "반복 기준",
            options=["1회성 교체 이상", "2회 이상 반복"],
            index=1,
            horizontal=True,
            key="piping_count_mode",
        )
        min_count = 1 if count_mode == "1회성 교체 이상" else 2
        f1, f2 = st.columns(2)
        with f1:
            line_keyword = st.text_input("Line Number 필터", key="piping_line_keyword")
        with f2:
            source_keyword = st.text_input("출처 필터", key="piping_source_keyword")

        candidate_df = build_piping_candidate_df(repeat_df if min_count >= 2 else build_piping_repeat_report_dataframe(occurrences_df))
        filtered_repeat_df = filter_piping_repeat_df(candidate_df, min_count=min_count, line_keyword=line_keyword, source_keyword=source_keyword)
        selected_lines = set(filtered_repeat_df.get("Line Number", pd.Series(dtype=str)).astype(str).tolist())
        filtered_occ_df = filter_piping_occurrences_by_lines(occurrences_df, selected_lines) if selected_lines else occurrences_df.copy()
        filtered_ex_df = filter_piping_excluded_by_lines(excluded_df, selected_lines, show_all=(min_count == 1 and not selected_lines))

        repeat_display_df = _condense_df_for_display(
            filtered_repeat_df,
            cols=["NO", "Line Number", "발생구분", "발생횟수", "발생년도", "최근발생일", "출처", "대표조치", "검토메모"],
            text_cols=["대표조치", "검토메모"],
            limit=200,
        )
        occ_display_df = _condense_df_for_display(
            filtered_occ_df,
            cols=["line_no", "event_date", "event_year", "sources", "titles", "details"],
            text_cols=["titles", "details"],
            limit=220,
        )
        ex_display_df = _condense_df_for_display(
            filtered_ex_df,
            cols=["line_no", "event_date", "source", "exclude_reason", "title", "detail"],
        ...
        filtered_excel_bytes, filtered_excel_name = make_piping_excel_bytes(
            ...
            occurrences_df=filtered_occ_df,
            excluded_df=filtered_ex_df,
        ...
        tabs = st.tabs(["반복배관후보", "occurrences", "excluded_review", "summary"])
        with tabs[0]:
            st.dataframe(_sanitize_df_for_display(repeat_display_df), use_container_width=True, height=380)
        with tabs[1]:
            st.dataframe(_sanitize_df_for_display(occ_display_df), use_container_width=True, height=380)
        with tabs[2]:
            st.dataframe(_sanitize_df_for_display(ex_display_df), use_container_width=True, height=380)
        with tabs[3]:
            st.dataframe(_sanitize_df_for_display(summary_df), use_container_width=True, height=240)
